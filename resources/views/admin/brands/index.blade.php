...
<div class="card-body">
    <a href="{{ url('/admin/brands/create') }}" class="btn btn-success btn-sm" title="Добавить новый бренд">
        ...
        <table class="table">
            ...
            <tbody>
                @foreach($brands as $brand)
                    ...
                    <td>{{ $brand->name ? Str::limit($brand->name, 50) : '' }}</td>
                    @if(isset($brand->image))
                    <td><img src="{{ url($brand->image)}}" alt="{{ $brand->image }}" width="50px;"></td>
                    @endif
                    <td>
                    <a href="{{ url('/admin/brands/' . $brand->id . '/edit') }}" title="Редактировать товар">
